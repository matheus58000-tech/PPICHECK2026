    ...
    if ($id_alvo === $id_usuario && ($acao === 'excluir' || $acao === 'bloquear')) {
        ...
        try {
            if ($acao === 'bloquear') {
                ...
                $_SESSION['msg_sucesso_lab'] = "Status do usuário alterado com sucesso!";
                redirectLab('view-lab', 'tab-usuarios');
                
            } elseif ($acao === 'excluir') {
                $conn->query("DELETE FROM Usuarios WHERE id_user = $id_alvo");
                $_SESSION['msg_sucesso_lab'] = "Usuário excluído permanentemente!";
                redirectLab('view-lab', 'tab-usuarios');
                ...
                $stmt_edit->execute();
                $_SESSION['msg_sucesso_lab'] = "Dados do usuário atualizados com sucesso!";
                redirectLab('view-lab', 'tab-usuarios');
                ...
                if (count($erros_lab) > 0) {
                    ...
                    $_SESSION['msg_sucesso_lab'] = "Usuário cadastrado com sucesso!";
                    redirectLab('view-lab', 'tab-usuarios'); 
                }
            }
        } catch (\Exception $e) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['acao_categoria'])) {
    ...
    try {
        ...
        if ($acao_cat === 'adicionar') {
            ...
            $_SESSION['msg_erro_lab'] = "ERRO: Não é possível excluir uma categoria que possui itens cadastrados nela.";
            redirectLab('view-lab', 'tab-categorias');
        }
    }
}
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="action-buttons">
                                    <button class="btn-action edit" onclick="abrirModalEditItem('<?php echo $dados_item_json; ?>')"><i class="bi bi-pencil-square"></i> Editar</button>
                                    <button class="btn-action delete" onclick="acaoItem('excluir', <?php echo $id_item; ?>)"><i class="bi bi-trash"></i> Excluir</button>
                                </td>
                            </tr>
                        <?php endwhile; else: ?>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="action-buttons">
                                    <button class="btn-action edit" onclick="abrirModalEditCategoria('<?php echo $dados_cat_json; ?>')"><i class="bi bi-pencil-square"></i> Editar</button>
                                    <button class="btn-action delete" onclick="acaoCategoria('excluir', <?php echo $cat['id_cat']; ?>)"><i class="bi bi-trash"></i> Excluir</button>
                                </td>
                            </tr>
                        <?php endforeach; else: ?>
...
<!-- Formulário Oculto para Ações Genéricas -->
...
<div id="modalEditUser" class="modal-overlay">
    <div class="modal-content form-container" style="max-width: 600px;">
        ...
        <form method="POST" action="FECHECKADM.php" style="margin-top: 15px;">
            ...
            <div class="form-actions" style="margin-top: 20px;">
                <button type="submit" class="btn-primary-action"><i class="bi bi-save"></i> Salvar</button>
...
<?php 
// SCRIPT DE REABERTURA AUTOMÁTICA DOS MODAIS (Caso dê erro de validação)
if (isset($abrir_modal_edit_item) && $abrir_modal_edit_item): 
?>
<script>
    window.addEventListener('DOMContentLoaded', () => {
        document.getElementById('edit_id_alvo_item').value = "<?php echo $_POST['id_alvo_item']; ?>";
        document.getElementById('edit_item_nome').value = "<?php echo addslashes($_POST['edit_item_nome']); ?>";
        document.getElementById('edit_item_qntd').value = "<?php echo $_POST['edit_item_qntd']; ?>";
        document.getElementById('edit_item_categoria').value = "<?php echo $_POST['edit_item_categoria']; ?>";
        document.getElementById('edit_item_descricao').value = "<?php echo str_replace(["\r", "\n"], ['\r', '\n'], addslashes($_POST['edit_item_descricao'])); ?>";
        document.getElementById('modalEditItem').style.display = 'flex';
    });
</script>
<?php endif; ?>
...
<script>
    window.addEventListener('DOMContentLoaded', () => {
        document.getElementById('edit_id_alvo_cat').value = "<?php echo $_POST['id_alvo_cat']; ?>";
        document.getElementById('edit_cat_nome').value = "<?php echo addslashes($_POST['edit_cat_nome']); ?>";
        document.getElementById('edit_cat_desc').value = "<?php echo str_replace(["\r", "\n"], ['\r', '\n'], addslashes($_POST['edit_cat_desc'])); ?>";
        document.getElementById('modalEditCategoria').style.display = 'flex';
    });
</script>
<?php endif; ?>
